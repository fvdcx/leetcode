...
public:
    int maxProfit(vector<int>& prices) {
          const int psz = prices.size();
          if (!psz) 
               return 0;
          // 0 one buy 1 one sell 2 two buy 3 tow sell
          int dp[2][4]={-prices[0], 0, INT_MIN ,0}, now = 1, last = 0;
          fo r(int i = 1; i < psz; i++)
          {
               dp[now][0] = max(dp[last][0], -prices[i] );
               dp[now][1] = max(dp[last][1], dp[last][0] + prices[i]);
               dp[now][2] = max(dp[last][2], dp[last][1] - prices[i]);
               dp[now][3] = max(dp[last][3], dp[last][2] + prices[i]);
               last = now;
               now ^= 1;
          }
          return max(dp[!(psz&1)][3],dp[!(psz&1)][1]);
    }
};
\end{Code}
